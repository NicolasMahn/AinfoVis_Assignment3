...
class DatabaseManager:
    def __init__(self, topic_dir: str, reset: bool = False, debug: bool = False, separate_in_chunks: bool = True,
                 chunk_separation_warning: bool = True, chunk_size: int = 8191):
    ...
    def split_text_into_chunks_by_size(self, text, num_chunks):
        chunks = []
    ...
    def split_text_into_chunks(self, text):
        num_tokens = self.num_tokens_from_string(text, "cl100k_base")
        if num_tokens > self.chunk_size:
            num_chunks = num_tokens // self.chunk_size + 1
            chunks = self.split_text_into_chunks_by_size(text, num_chunks)
        else:
            chunks = [text]
